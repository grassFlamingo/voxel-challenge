...
class Camera:
    def __init__(self, window, up):
        self._window = window # type: ti.GUI
    ...
    def update_camera(self, key_list):
        res = self._update_by_wasd(key_list)
        res = self._update_by_mouse() or res
        return res

    def _update_by_mouse(self):
        win = self._window # type: ti.GUI
        if not self.mouse_exclusive_owner or not win.is_pressed(ti.GUI.LMB):
            self._last_mouse_pos = None
            return False
    ...
    def _update_by_wasd(self, key_list):
        win = self._window # type: ti.GUI
...
class Scene:
    ...
    def finish(self):
        ...
        while self.window.running:
            ...
            if self.camera.update_camera(key_list):
                self.renderer.set_camera_pos(*self.camera.position)
